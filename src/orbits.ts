...
class SpaceObjectData {
    ...
    getPlanetOrbit3DPoints(numPoints: number): Vector3[] {
        // const theta: number[] = Array.from({ length: numPoints }, (_, i) => (360 * i) / (numPoints - 1));
    ...
    getPoint(angle: number): [number, number, number] {
        ...
        return [
            ...
            (yi + this._basePoint[1]) * this._scale,
            (zi + this._basePoint[2]) * this._scale,
        ];
    }
